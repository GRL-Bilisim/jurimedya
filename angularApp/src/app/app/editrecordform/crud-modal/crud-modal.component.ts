              ...
              if (result.value) { 
                ...
                if (durum.formid=='401') 
                {
                  ...
                  for (let i = 0; i < durum.tabloSecililer.length; i++) {
                    ...
                    ambalaj_adi = ambalaj_adi.replace(' ','%');
                    var kalanmiktars = durum.tabloSecililer[i].kalanmiktars;
                    var kod_urun = durum.tabloSecililer[i].kod_urun;
                   var master = durum.id;
                    var firma_kod = durum.tabloSecililer[i].firmaidC;

                    const girdikal=localStorage.getItem('url') + "angular/dataservis.php?page_id=cekisiparisekle&firmakodu="+firma_kod+"&ch_kodu="+ch_kodu+"&kod_urun="+kod_urun+"&kalemid="+kalemid+"&firma_siparis_no="+firma_siparis_no+"&sip_id="+sip_id+"&koli_ici_adet="+koli_ici_adet+"&ambalaj_adi="+ambalaj_adi+"&kalanmiktars="+kalanmiktars+"&mastervalue="+master+"&userid="+localStorage.getItem('userid');
