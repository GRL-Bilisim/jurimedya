  <?php
  if ($_GET['page_id']=='sevkemriolustur'){
...
$rezervedilecekmiktar='';
 ...
 $count= $uquery->rowCount();
   ...
   if($miktar=='0'){
        ...
        $tarih=$tarihim;
        $hareketturu=iconv( "UTF-8","ISO-8859-9",'Sevk İzni Bekliyor');
        $hareketsaati=$saat;
        $miktardb=$rezervedilecekmiktar;
        $belgeno='';
        $aciklama=$okut[sip_id].iconv("UTF-8","ISO-8859-9"," nolu sipariş fişi üzerinden sevk yapılacak.");
        $islemyeri='1';
...
if($miktar<=$okutyes[kalan]){
   ...
   else  if ($okutyes[kalan]<$okut[kesme_isemri_gelecek_miktar])
    {
         echo "[{\"status\":\"NO\"}]";
    ...
    else if ($okutyes[kalan]>=$okut[kesme_isemri_gelecek_miktar])
    {
       ...
       $miktardb=$rezervedilecekmiktar;
       $belgeno='';
       $aciklama=$okut[sip_id].iconv("UTF-8","ISO-8859-9"," nolu sipariş fişi üzerinden sevk yapıldı.");
       $islemyeri='1';
       $olusturanform='278';
       $sipid = $okut[index_siparis_izleme];
